...
## Short write-up
What it is: A self-hosted dashboard that turns a personal stock watchlist into transparent BUY/HOLD/SELL calls. It pulls daily OHLCV prices and fundamentals from Yahoo Finance, computes five technical indicators (SMA, EMA, RSI, MACD, Bollinger Bands) plus P/E and earnings growth, and blends them into a single -1..+1 score with a visible per-indicator breakdown and a confidence measure, so nothing is a black box. A walk-forward, no-look-ahead backtester shows how the strategy would have performed over the past year versus buy-and-hold. A static "Research" tab keeps a separate academic semiconductor study clearly labeled and isolated from the live pipeline.

The problem it solves: Most retail "signal" tools are opaque; you get an arrow, not a reason. This dashboard exposes every weight and cutoff in config.py and shows the math behind each call, while explicitly refusing to be a trading tool (no broker integration, no order placement, no auth; it's a local research instrument).

How we used Kiro:
We used Kiro's spec-driven workflow to go from idea to working app without losing the thread. A PROJECT_CONTEXT.md steering document captured hard constraints up front (research-only, never trade, no broker/API keys), so every generated feature respected them automatically. Kiro broke the build into an 8-step task plan (config, data layer, indicators, scoring, backtester, API/UI, wiring), which kept the data, analysis, and backtest pipeline consistent across files generated in separate sessions.

Kiro's persistent context also meant real iteration got recorded, not lost: when live signals clustered near HOLD, the threshold was retuned and the reasoning logged inline in config.py; when Yahoo started 429-ing, cache TTLs and request throttling were tightened the same way. That running memory of why things changed, not just what, is what let the project evolve safely across sessions.
