...
def score_sma(ind: dict) -> float:
    return _trend_score(_num(ind.get("Close")), _num(ind.get("SMA_short")),
                        _num(ind.get("SMA_long")))


def score_ema(ind: dict) -> float:
    return _trend_score(_num(ind.get("Close")), _num(ind.get("EMA_short")),
                        _num(ind.get("EMA_long")))


def score_macd(ind: dict) -> float:
    """Bullish when the MACD line is above its signal and above zero."""
    line = _num(ind.get("MACD"))
    sig = _num(ind.get("MACD_signal"))
...
def score_bollinger(ind: dict) -> float:
    ...
    upper = _num(ind.get("BB_upper"))
    lower = _num(ind.get("BB_lower"))
