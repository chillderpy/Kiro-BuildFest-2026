...
function darkLayout(extra) {
  return Object.assign({
    ...
    hovermode: "x unified",
    // FIX: click-drag now PANS the chart left/right (was locked).
    dragmode: "pan",
    // Make the (now-visible) toolbar readable on the dark theme.
    modebar: { bgcolor: "rgba(0,0,0,0)", color: THEME.text, activecolor: THEME.blue },
  }, extra || {});
}

// FIX: charts are interactive now - scroll to zoom, drag to pan, and a toolbar
// with zoom/pan/reset. displayModeBar was false before, which hid the controls
// and made the chart feel frozen.
const PLOT_CONFIG = {
  responsive: true,
  scrollZoom: true,
  displayModeBar: true,
...
function drawPriceChart(data) {
  ...
  // FIX: explicit height keeps the plot from leaving empty space in the panel.
  Plotly.newPlot(el, traces, darkLayout({ height: 360 }), PLOT_CONFIG);
}
...
function drawEquityChart(data) {
  ...
  // FIX: explicit height (no empty space below the equity chart).
  const layout = darkLayout({ height: 280, margin: { l: 60, r: 12, t: 10, b: 36 } });
  if (start !== null) {
    layout.shapes = [{
  ...
  document.querySelectorAll("#timeframe .tf-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      document.querySelectorAll("#timeframe .tf-btn")
        .forEach((b) => b.classList.toggle("active", b === btn));
      // FIX: timeframe switch redraws only the chart (loadChartOnly), it does
      // NOT reload the whole detail panel.
      loadChartOnly();
    });
  });
