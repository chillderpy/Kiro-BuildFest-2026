...
def main():
    ...
    print("GET /api/research         ->", r.status_code)
    if r.status_code == 200:
        d = r.get_json()
        print(f"   static={d.get('is_static')} stocks={len(d.get('stocks', []))} "
              f"rules={len(d.get('decision_rules', []))} limits={len(d.get('limitations', []))}")
    ok &= r.status_code == 200 and r.get_json().get("is_static") is True

    r = client.get(f"/api/backtest/{tkr}")
    print(f"GET /api/backtest/{tkr}    ->", r.status_code)
    if r.status_code == 200:
        d = r.get_json()
        if d.get("ok"):
            print(f"   {tkr}: window {d['start_date']}..{d['end_date']} "
                  f"({d['trading_days']}d) | strategy {d['total_return_pct']}% "
                  f"vs B&H {d['buy_and_hold_return_pct']}% | beat={d['beat_buy_and_hold']} "
                  f"| trades={d['num_trades']} win={d['win_rate_pct']}% "
                  f"maxDD={d['max_drawdown_pct']}%")
        else:
            print("   backtest could not run:", d.get("message"))
    ok &= r.status_code == 200
