...
- Scoring -1..+1: BUY >= +0.15, SELL <= -0.15, HOLD otherwise (tightened from
  +/-0.30 so signals aren't all HOLD)
...
- Rate limit: ~5s gap between live calls (skipped on cache hit) - conservative to avoid 429s
- Disk cache (JSON): prices TTL ~30 min, company info TTL ~6 h
